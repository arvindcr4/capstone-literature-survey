# Literature Survey — RL for LLMs: A Systematic GRPO Scaling Study

**Group 6** | MTech in Data Science & AI | Semester 3 Capstone | PES University, Bengaluru

## Team
- Madhu Kumara L
- Sandhya Jeyaraj
- Mohammad Rafi
- Arumugam Chetty K
- Arvind CR
- Dhruva N

## Contents

### Foundation Chapter (Chapter 2)
- `foundation_chapter.tex` — Complete compiled chapter
- `foundation_chapter.pdf` — Compiled PDF

### Individual Sections
| File | Topic |
|------|-------|
| `chapter2_foundation.tex` | Chapter wrapper |
| `section_2_2_rlhf.tex` | Reinforcement Learning from Human Feedback |
| `section_2_3_ppo.tex` | Proximal Policy Optimization |
| `section_2_4_dpo.tex` | Direct Preference Optimization |
| `section_2_5_grpo.tex` | Group Relative Policy Optimization |
| `section_2_6_deepseek_r1.tex` | DeepSeek-R1 |
| `section2_7_peft.tex` | Parameter-Efficient Fine-Tuning (LoRA) |
| `section_2_8_mathematical_reasoning_benchmarks.tex` | Math Reasoning Benchmarks (GSM8K, MATH) |
| `section_2_9_related_work_rl_reasoning.tex` | Related Work (DeepScaleR, SimpleRL-Zoo, TinyZero, DeepCoder) |
| `section_2_10_infrastructure_rl_training.tex` | Infrastructure for RL Training (Tinker/Atropos) |
| `section_2_11_scaling_laws.tex` | Scaling Laws for RL Fine-Tuning |
